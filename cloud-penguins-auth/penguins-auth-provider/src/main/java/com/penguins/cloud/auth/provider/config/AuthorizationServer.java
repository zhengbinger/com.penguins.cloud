...
public class AuthorizationServer extends AuthorizationServerConfigurerAdapter {
  ...
  /**
   * 授权码服务采用数据库模式  JdbcAuthorizationCodeServices
   *
  ...
  /**
   * 配置授权服务器端点的非安全功能
   * 例如令牌存储，令牌自定义，用户批准和授予类型。
   * 默认情况下，您不需要做任何事情，除非需要密码授予，在这种情况下，
   * 需要提供 AuthenticationManager
   *
  ...
  /**
   * 配置 ClientDetailsServiceConfigurer 负责查找 ClientDetails
   * 例如声明单个客户及其属性。
   * 注意，除非将 AuthenticationManager 提供给 configure（AuthorizationServerEndpointsConfigurer），
...
//            .secret()
   ...
   * /oauth/token 令牌断点
   * /oauth/confirm-access 用户确认授权提交端点
